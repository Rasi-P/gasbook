...
function emptyItem(types: CylinderType[]): SaleItem {
  const t = types[0];
  return { cylinder_type: t?.id ?? 0, quantity: 1, rate: String(t?.selling_price ?? ''), empty_returned: 0 };
}
...
export default function Sales() {
  ...
  const [customerSuggestions, setCustomerSuggestions] = useState<{ id: number; name: string; phone: string; address: string; pending_balance: number; empties_owed: number }[]>([]);
  ...
  useEffect(() => {
    Promise.all([api.get('/cylinder-types/'), api.get('/locations/')])
      .then(([tr, lr]) => {
        ...
        setItems([emptyItem(types)]);
      })
      .catch(() => setItems([]));
  }, []);

  useEffect(() => {
    if (items.length === 0 && cylinderTypes.length > 0) {
      setItems([emptyItem(cylinderTypes)]);
    }
  }, [cylinderTypes, items.length]);
  ...
  function updateItem(index: number, patch: Partial<SaleItem>) {
    setItems((prev) => prev.map((item, i) => {
      ...
      // auto-fill rate when cylinder changes
      if (patch.cylinder_type !== undefined) {
        const t = cylinderTypes.find((c) => c.id === patch.cylinder_type);
        updated.rate = String(t?.selling_price ?? updated.rate);
      }
      return updated;
    }));
  }
  ...
  function addItem() {
    setItems((prev) => [...prev, emptyItem(cylinderTypes)]);
  }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      setCustomerName(''); setPhone(''); setAddress(''); setSelectedCustomerId(null);
        ...
        <form onSubmit={handleSubmit} className="form-stack">
                    ...
                    <div className="dropdown-list">
                      {customerSuggestions.map((c) => (
                        <button key={c.id} type="button"
                          onClick={() => { setCustomerName(c.name); setPhone(c.phone); setAddress(c.address); setSelectedCustomerId(c.id); setCustomerSuggestions([]); }}
          ...
          <div className="card">
            ...
            <div className="form-stack">
              {items.map((item, i) => {
                const defaultRate = cylinderTypes.find((c) => c.id === item.cylinder_type)?.selling_price;
                const isCustomRate = item.rate !== '' && Number(item.rate) !== defaultRate;
                return (
                  <div key={i} style={{ background: 'var(--surface-muted)', borderRadius: '8px', padding: '14px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginTop: '10px' }}>
                      ...
                      <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)', fontWeight: 600 }}>Empty returned:</span>
                      <input type="number" min="0" max={item.quantity} value={item.empty_returned}
                        onChange={(e) => updateItem(i, { empty_returned: Number(e.target.value) || 0 })}
